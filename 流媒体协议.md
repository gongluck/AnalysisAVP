...
- **Message**（消息）
  ...
  - **Length**（⻓度）：是指**Message Payload**（消息负载）即⾳视频等信息的数据的⻓度，**3**个字节
...
- **Chunk Format**（块格式）
  ...
  - **Basic Header**：
    ...
    - **Basic Header**的⻓度可能是**1，2，或3个字节**，其中**chunk type**的⻓度是固定的**2bit**，**Basic Header**是变⻓的，其⻓度取决于**CSID**的⼤⼩，在⾜够存储这两个字段的前提下最好⽤尽量少的字节从⽽减少由于引⼊Header增加的数据量。

    - **RTMP**协议最多⽀持**65597**个⽤户⾃定义**chunk stream ID**，范围为**[3，65599]** ，**ID 0，1，2**被协议规范直接使⽤，其中**ID值为0，1分表表示了Basic Header占⽤2个字节和3个字节**。
